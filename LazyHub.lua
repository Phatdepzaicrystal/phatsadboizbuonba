...
--button images logo
ImageButton.Image = "rbxassetid://102940522579235"
...
function dragify(Frame, object)
	...
	function updateInput(input)
		...
		if _G['à¸—à¸³à¹ƒà¸«à¹‰ UI à¹„à¸¡à¹ˆà¸­à¸­à¸ à¸‚à¸­à¸š'] then
...
local Windown = library.Create({
	Title = "may anh trai nay hiphop", Logo = "rbxassetid://102940522579235",
...
spawn(function()
    ...
    function StopTween(target)
        if not target then
	    	...
	    	TPP(game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame)
            wait()
            if game:GetService("Players").LocalPlayer.Character.HumanoidRootPart:FindFirstChild("BodyClip") then
                game:GetService("Players").LocalPlayer.Character.HumanoidRootPart:FindFirstChild("BodyClip"):Destroy()
            end
...
Page1.CreateToggle({
	...
	Value = false,
	Callback = function(v)
	    _G.AutoFarmChestHop = v
	    StopTween(_G.AutoFarmChestHop)
		print(v)
	end,
})
...
spawn(function()
    while wait() do 
        if _G.AutoFarmChest then
            local chestFound = false -- Biáº¿n kiá»ƒm tra sá»± tá»“n táº¡i cá»§a ChestModels
            for i, v in pairs(game:GetService("Workspace"):GetChildren()) do 
                if v.Name:find("ChestModels") then
                    chestFound = true -- ÄÃ¡nh dáº¥u ráº±ng cÃ³ Ã­t nháº¥t 1 ChestModels tá»“n táº¡i
            ...
            -- Náº¿u khÃ´ng tÃ¬m tháº¥y báº¥t ká»³ "ChestModels" nÃ o
                ...
                if v.Name:find("ChestModels") then
                    ...
                    if game:GetService("Workspace"):FindFirstChild(v.Name) then
                        if (v.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 5000 + _G.MagnitudeAdd then
                            ...
                            until AutoFarmChestHop == false or not v.Parent
                            topos(game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame)
                            _G.MagnitudeAdd = _G.MagnitudeAdd + 1500
                            break
                        end
...
Module.EquipTool = setmetatable({}, {
    __call = function(tbl, itemName, useToolTip)
        local playerChar = Player.Character
        if Module.IsAlive(playerChar) then
            local currentTool = tbl.Equipped

            if currentTool and currentTool.Parent and currentTool[useToolTip and "ToolTip" or "Name"] == itemName then
                if currentTool.Parent ~= playerChar then
                    playerChar:WaitForChild("Humanoid"):EquipTool(currentTool)
                end
                return
            end

            if itemName and not useToolTip then
                local foundTool = Player.Backpack:FindFirstChild(itemName)
                if foundTool then
                    tbl.Equipped = foundTool
                    playerChar:WaitForChild("Humanoid"):EquipTool(foundTool)
                end
                return
            end

            local toolTipName = useToolTip and itemName or _G.SelectWeapon
            for _, tool in ipairs(Player.Backpack:GetChildren()) do
                if tool:IsA("Tool") and tool.ToolTip == toolTipName then
                    tbl.Equipped = tool
                    playerChar:WaitForChild("Humanoid"):EquipTool(tool)
                    break
                end
            end
        end
...
local RegisterAttackEvent = Net:WaitForChild("RE/RegisterAttack")
local RegisterHitEvent = Net:WaitForChild("RE/RegisterHit")

local EnemyFolder = Workspace:WaitForChild("Enemies")
local CharacterFolder = Workspace:WaitForChild("Characters")

local function CollectEnemies(targetList, folder)
    local selectedTarget = nil

    for _, entity in ipairs(folder:GetChildren()) do
        local headPart = entity:FindFirstChild("Head")

        if headPart and Module.IsAlive(entity) and Player:DistanceFromCharacter(headPart.Position) < FastAttack.Range then
            if entity ~= Player.Character then
                table.insert(targetList, { entity, headPart })
                selectedTarget = headPart
            end
        end
    end
...
function FastAttack:ExecuteAttack(target, targetList)
    RegisterAttackEvent:FireServer(_G.FastAttackDelay or 0.05)
    RegisterHitEvent:FireServer(target, targetList)
end

function FastAttack:FindAndAttackNearest()
    local potentialTargets = {}

    local firstTarget = CollectEnemies(potentialTargets, EnemyFolder)
    local secondTarget = CollectEnemies(potentialTargets, CharacterFolder)

    if #potentialTargets > 0 then
        self:ExecuteAttack(firstTarget or secondTarget, potentialTargets)
    else
        task.wait(1)
    end
end

function FastAttack:HandleBladeHits()
    local currentTool = Module.IsAlive(Player.Character) and Player.Character:FindFirstChildOfClass("Tool")

    if currentTool and currentTool.ToolTip ~= "Gun" then
        self:FindAndAttackNearest()
    else
        task.wait(0.5)
    end
end
...
task.spawn(function()
    while task.wait(_G.FastAttackDelay or 0.125) do
        if _G.FastAttack and not _G.AutoFarmFruitMastery and not _G.AutoFarmGunMastery and (tick() - Module.AttackCooldown) >= 1 then
            FastAttack:HandleBladeHits()
        end
    end
end)
...
Page2.CreateDropdown({
	...
	Value = "0.2",
	List = {0, 0.1, 0.150, 0.165, 0.175, 0.2, 0.25 ,0.3},
	Callback = function(v)
        ...
        pcall(function()
            while wait() do
                if game.Workspace._WorldOrigin.Locations:FindFirstChild('Mirage Island') then
                    Mirragecheck:Set('âœ… Mirage Island is Spawning')
                else
                    Mirragecheck:Set('âŒ Mirage Island Not Spawn') 
                 end
            end
        end)
    end)
    ...
    pcall(function()
        while wait() do
            if game:GetService("Workspace").Map:FindFirstChild('KitsuneIsland') then
                SetCheeask:Set('âœ…: Kitsunen Spwan')
            else
                SetCheeask:Set('âŒ: Kitsunen Not Found')
            end
        end
    end)
end)
                            ...
                            if game:GetService("Workspace").Enemies:FindFirstChild("Fish Crew Member") then
                                if v.Name == "Fish Crew Member" then
                                    ...
                                    repeat task.wait(_G.FastAttackDelay)
                                        AutoHaki()
                                        EquipWeapon(_G.SelectWeapon)
                                        TPP(v.HumanoidRootPart.CFrame * Pos)
                                        AttackNoCD()
                                        _G.SeaSkill = false
                        ...
                        for i, v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                            if workspace.Enemies:FindFirstChild("FishBoat") then
                                repeat task.wait()
                                    ...
                                    TPP(BoatCFrame1 * CFrame.new(0, 20, -20))
                                    Skillaimbot = true
                        ...
                        for i, v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                            if workspace.Enemies:FindFirstChild("PirateBrigade") then
                                repeat task.wait()
                                    ...
                                    TPP(BoatCFrame2 * CFrame.new(0, 20, -20))
                                    Skillaimbot = true
                                    AimBotSkillPosition = v.VehicleSeat.Position
                        ...
                        for i, v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                            if workspace.Enemies:FindFirstChild("PirateGrandBrigade") then
                                repeat task.wait()
                                    ...
                                    TPP(BoatCFrame3 * CFrame.new(0, 20, -20))
                                    Skillaimbot = true
                                    AimBotSkillPosition = v.VehicleSeat.Position
                        ...
                        if game:GetService("Workspace"):FindFirstChild("SeaBeasts") then
                            for i,v in pairs(game:GetService("Workspace").SeaBeasts:GetChildren()) do
                                ...
                                repeat wait()
                                    ...
                                    Skillaimbot = true
                                    AimBotSkillPosition = v.HumanoidRootPart.Position
                                    TPP(CFrameSeaBeast * CFrame.new(0,500,0))
...
Page5.CreateToggle({
	Name = "Auto Farm Fish Crew Members",
	Dis =  "Fish Crew Members",
	Value = false,
	Callback = function(v)
...
if World1 then
Page11.CreateLable({
	Name = "work on Second sea or Thirdsea"
})

    elseif World3 or World2 then
Page11.CreateLable({
	Name = "Raid"
})
...
Page13.CreateButton({
	Name = "Buy Reset Stats [ Use à¸¦â€™2,500 ]",
	Callback = function()
...
Page13.CreateButton({
	Name = " Buy Random Race [ Use à¸¦â€™3000 ]",
	Callback = function()
    ...
    task.spawn(function()
        while task.wait() do
            pcall(function()
                if game:GetService("Lighting").Sky.MoonTextureId=="http://www.roblox.com/asset/?id=9709149431" then
                    FM:Set("à¹ÂŸÂŒâ€¢ Full Moon 100%")
                elseif game:GetService("Lighting").Sky.MoonTextureId=="http://www.roblox.com/asset/?id=9709149052" then
                    FM:Set("à¹ÂŸÂŒâ€“ Full Moon 75%")
                elseif game:GetService("Lighting").Sky.MoonTextureId=="http://www.roblox.com/asset/?id=9709143733" then
                    FM:Set("à¹ÂŸÂŒâ€” Full Moon 50%")
                elseif game:GetService("Lighting").Sky.MoonTextureId=="http://www.roblox.com/asset/?id=9709150401" then
                    FM:Set("à¹ÂŸÂŒÂ˜ Full Moon 25%")
                elseif game:GetService("Lighting").Sky.MoonTextureId=="http://www.roblox.com/asset/?id=9709149680" then
                    FM:Set("à¹ÂŸÂŒÂ˜ Full Moon 15%")
                else
        ...
        spawn(function()
            pcall(function()
                while wait() do
        if game.Workspace._WorldOrigin.Locations:FindFirstChild('Mirage Island') then
        MirageStatus:Set('Â Mirage Island is Spawning')
        else
        MirageStatus:Set('Â Mirage Island Not Spawn') 
        end
          
        if game:GetService("Workspace").Map:FindFirstChild('KitsuneIsland') then
        KitsuneStatus:Set('Â Kitsune Island is Spawning')
        else
        KitsuneStatus:Set('Â Kitsune Island Not Spawn') 
        end

        for i,v in pairs(game.Workspace:GetChildren()) do
        if string.find(v.Name, "Fruit") then
        FindFruit:Set('ÂŸÂÂ Find '..v.Name)
        else
        FindFruit:Set('ÂÂ Nothing') 
        end
        end
                end
            end)
            ...
            while wait(5) do
                ...
                if _G.AutoBuyLegendarySword or _G.AutoBuyEnchancementColour then
                        if GetLegendarySwordDealer() then
                            SwordDealer:Set('Â Legendary Sword Dealer is Spawning')
                        else
...
Page16.CreateDropdown({
	Name = "Server Region",
	Value = "Singapore",
	List = {"Singapore", "United States","Germany","France","Australia","???"},
	Callback = function(v)
	RegionHop = v
		print(v)
	end,
...
Page16.CreateButton({
	...
	while wait() do
        ...
        module:Teleport(game.PlaceId, RegionHop)
    end
	end,
})
